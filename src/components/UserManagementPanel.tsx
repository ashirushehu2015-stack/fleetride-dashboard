...
export default function UserManagementPanel({
  ...
  const getCategoryColor = (cat: string) => {
    switch(cat) {
      case 'SYSTEM': return 'bg-purple-500/10 text-purple-400 border border-purple-500/20';
      ...
      case 'RIDER': return 'bg-emerald-500/10 text-emerald-400 border border-emerald-500/20';
  ...
  return (
    <div className="bg-zinc-950 border border-zinc-900 rounded-2xl h-full flex flex-col text-zinc-100 overflow-hidden" id="user-management-module">
      
      {/* MODULE HEADER */}
      <div className="p-4 bg-zinc-950 border-b border-zinc-900 flex items-center justify-between gap-2 shrink-0">
        <div className="flex items-center gap-2">
          <div className="p-1.5 bg-amber-500/10 text-amber-500 rounded-lg">
            <Shield size={16} />
          </div>
          <div>
            <h3 className="text-sm font-black tracking-tight text-white">National Fleet Console</h3>
            <p className="text-[9px] text-zinc-500 font-bold uppercase tracking-wider">Double-Sided Identity & Auditing Hub</p>
          </div>
        </div>

        {/* ACTIVE PROFILE PILOT BADGE */}
        <div className="flex items-center gap-2 bg-zinc-900/50 border border-zinc-800 px-2.5 py-1 rounded-xl text-right">
          <div className="hidden sm:block">
            <div className="text-[8px] text-zinc-500 font-bold uppercase tracking-widest leading-none">Acting Agent</div>
            <div className="text-[10px] font-black text-white">{activeProfile.name}</div>
          </div>
          <img 
            src={activeProfile.avatar} 
            alt="active avatar" 
            className="w-5.5 h-5.5 rounded-full object-cover border border-zinc-700 shrink-0"
          />
          <span className="text-[10px] px-1.5 py-0.5 rounded bg-zinc-800 text-zinc-400 font-mono font-bold leading-none">
      ...
      <div className="px-3 py-2 bg-zinc-900/30 border-b border-zinc-900 flex gap-1.5 overflow-x-auto shrink-0 scrollbar-none">
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition cursor-pointer shrink-0 ${
            subTab === 'passengers' ? 'bg-white text-zinc-950' : 'text-zinc-400 hover:text-white hover:bg-zinc-900'
          }`}
        >
          <User size={13} />
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition cursor-pointer shrink-0 ${
            subTab === 'drivers' ? 'bg-white text-zinc-950' : 'text-zinc-400 hover:text-white hover:bg-zinc-900'
          }`}
        >
          <Car size={13} />
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition cursor-pointer shrink-0 ${
            subTab === 'admins' ? 'bg-white text-zinc-950' : 'text-zinc-400 hover:text-white hover:bg-zinc-900'
          }`}
        >
          <Shield size={13} />
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition cursor-pointer shrink-0 ${
            subTab === 'control' ? 'bg-amber-500 text-zinc-950 font-black shadow' : 'text-amber-500 hover:text-amber-400 hover:bg-amber-500/5'
          }`}
        >
          <Activity size={13} />
          Admin Control Tower
        ...
        <button
          ...
          className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 transition cursor-pointer shrink-0 ${
            subTab === 'trips' ? 'bg-white text-zinc-950 shadow' : 'text-zinc-400 hover:text-white hover:bg-zinc-900'
          }`}
        >
          <Calendar size={13} />
      ...
      {subTab !== 'control' && subTab !== 'trips' && (
        <div className="p-3 bg-zinc-950 border-b border-zinc-900 flex items-center gap-2 shrink-0">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full bg-zinc-900 border border-zinc-800 rounded-xl py-2 pl-9 pr-4 text-xs text-white focus:outline-none focus:border-zinc-700 placeholder-zinc-500"
            />
          </div>
          {subTab === 'passengers' && (
            <button
              onClick={() => setShowPassengerModal(true)}
              className="px-3 py-2 bg-zinc-900 border border-zinc-800 hover:border-zinc-700 rounded-xl text-xs font-bold text-white flex items-center gap-1 cursor-pointer transition shrink-0"
            >
              <Plus size={14} />
              Add Rider
            </button>
